...
class GolikeClient {
    ...
    public:
     explicit GolikeClient(const std::string& token,const std::string& platform,const std::string& cookie ="",const std::string& type ="like");
     ~GolikeClient();
     GolikeJob ME() const;
...
class INSTAGRAM : public PlatformStrategy {
    ...
    public:
     ...
     ~INSTAGRAM()override = default;
     std::pair<std::string, std::string> GET_DATA() const;
     std::vector<GolikeJob> ACCOUNT() const override;
     std::vector<GolikeJob> GETJOB(long long account_id) const override;
     std::vector<GolikeJob> COMPLE(const int& ads_id,const int& account_id,const std::string& type,const std::string& user_id,const std::string& object_id) const override;
     std::vector<GolikeJob> SKIP(const int& ads_id, const std::string& object_id, const int& account_id, const std::string& type) const override;
     std::string LIKE(const std::string& av_id, const std::string& dtsg, const std::string& media_id) const;
     std::string FOLLOW(const std::string& target_id, const std::string& av_id, const std::string& dtsg) const;
};
class FACEBOOK : public PlatformStrategy {
    ...
    public:
     ...
     std::pair<std::string, std::string> GET_DATA(const std::string& cookies) const;
     std::vector<GolikeJob> ACCOUNT() const override;
     std::vector<GolikeJob> GETJOB(long long account_id) const override;
     std::vector<GolikeJob> COMPLE(const int& ads_id,const int& account_id,const std::string& type,const std::string& user_id,const std::string& object_id) const override;
     std::vector<GolikeJob> SKIP( const int& ads_id, const std::string& object_id, const int& account_id, const std::string& type) const override;
     std::pair<std::string,std::string> Feedback_ID(const std::string& url, const std::string& action) const;
     std::string LIKE(const std::string& url,const std::string& reaction,const std::string& actor_ID,const std::string& cookie) const;
     std::string FOLLOW(const std::string& url,const std::string& actor_ID,const std::string& cookie) const;
};
#endif
